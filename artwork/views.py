...
from artwork.forms import LoginForm, RegistrationForm, UserPasswordResetForm, UserSetPasswordForm, UserPasswordChangeForm
...
def social_login_success(request):
    ...
    if user.is_authenticated:
        social_account = SocialAccount.objects.filter(user=user, provider__in=['google', 'apple']).first()
        ...
        if social_account:
            ...
            if social_account.date_joined == social_account.last_login:
                try:
                    ...
                    messages.success(request, f"Welcome! Your account has been created using {social_account.provider}.")
                except Exception as e:
                    print(f"Error during social login processing: {str(e)}")
                    messages.warning(request, "We encountered an issue setting up your account. Please contact support if you experience any problems.")
            else:
                messages.info(request, f"Welcome back! You've successfully logged in with {social_account.provider}.")
        else:
